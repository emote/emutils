...
var propCheck = new RegExp("^(" + reservedPropNames.join('|') + ")\_*$", "i");
...
/**
 Get the CDM property name for a string, escaping it if need be by appending "_"
 @method getCdmPropertyName
 @param {String} name
 @return {String} the CDM type name
 **/
function getCdmPropertyName(name) {
    return isReservedPropertyName(name) ? name  + "_" : name;
}
